...
/**
 * Halaman Roadmap (gaya roadmap.sh): pilih roadmap dari #roadmap, lihat
 * urutan belajar materi yang terhubung. Data roadmaps.json dimuat lazy.
 */
export default function RoadmapPage() {
  const { fileMap } = useSiteData();
  const [data, setData] = useState<RoadmapsData | null>(null);
  ...
  useEffect(() => {
    ...
    fetchRoadmaps()
      .then((roadmaps) => {
        if (cancelled) return;
        setData(roadmaps);
        setSelectedId(roadmaps.roadmaps[0]?.id ?? null);
      })
      .catch((err) => {
  ...
  const steps = useMemo(() => {
    if (!selected) return [];
    return selected.stepIds
      .map((id) => {
        const entry = fileMap.get(id);
        return entry
          ? { id, title: entry.title, folder: entry.relativePath.split('/').slice(0, -1).join('/') }
          : null;
      })
      .filter((step): step is { id: string; title: string; folder: string } => step !== null);
  }, [selected, fileMap]);
      ...
      <div className="min-w-0 flex-1 overflow-y-auto">
        {selected ? (
          <div className="p-5 sm:p-7">
            ...
            <RoadmapFlow roadmap={selected} steps={steps} />
          </div>
        ) : (
